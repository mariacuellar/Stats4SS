...
We could also make a histogram of gender, but it's less interesting.
```{r}
# Histogram of gender.
hist(as.numeric(as.factor((dat$gender))), main="Histogram of gender", xlab="Gender")
```
...
Make an exportable figure that will be saved in the directory you're currently in. Not sure which directory you're in? Check with `r, eval=FALSE getwd()` and if you need to switch you can just use `r, eval=FALSE setwd()`.
...
How is age related to number of convictions?
```{r}
